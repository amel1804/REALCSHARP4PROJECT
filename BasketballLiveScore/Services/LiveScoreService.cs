    ...
    public class LiveScoreService : ILiveScoreService
    {
        ...
        public async Task<bool> StartClockAsync(int matchId)
        {
            try
            {
                ...
                // V�rifier que le match peut �tre d�marr�
                // Utilisation explicite du namespace pour �viter l'ambigu�t�
                ...
                {
                    _logger.LogWarning("Le match {MatchId} ne peut pas �tre d�marr� (status: {Status})", matchId, match.Status);
                    return false;
                }
